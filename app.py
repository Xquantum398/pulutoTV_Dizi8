...
# app.py DOSYASINDAKİ MEVCUT VavooResolver SINIFINI KOMPLE SİLİP,
# BU GÜNCELLENMİŞ VERSİYONU YAPIŞTIRIN.
...
class VavooResolver:
    def __init__(self):
        ...
        # --- DEĞİŞİKLİK BURADA ---
        # Önbellek süreleri isteğiniz üzerine güncellendi:
    ...
    def getAuthSignature(self):
        """
        Vavoo'dan kimlik doğrulama imzasını alır.
        Eğer birden fazla istek aynı anda gelirse, sadece biri imzayı alır, diğerleri bekler.
        """
        with self.lock:
            ...
            app.logger.info("Yeni Vavoo imzası alınıyor...")
            headers = { "accept": "application/json", "content-type": "application/json; charset=utf-8" }
            data = {
                "token": "tosFwQCJMS8qrW_AjLoHPQ41646J5dRNha6ZWHnijoYQQQoADQoXYSo7ki7O5-CsgN4CH0uRk6EEoJ0728ar9scCRQW3ZkbfrPfeCXW2VgopSW2FWDqPOoVYIuVPAOnXCZ5g",
                "reason": "app-blur", "locale": "de"
            }
            try:
                resp = self.session.post("https://www.vavoo.tv/api/app/ping", json=data, headers=headers, timeout=15)
    ...
    def resolve_vavoo_link(self, link, verbose=False):
        """
        Vavoo linkini çözer. İmza yoksa önce onu alır ve sonra linki çözer.
        """
        if link in self.resolved_link_cache:
            app.logger.info(f"Çözülmüş Vavoo linki önbellekten bulundu: {link}")
            return self.resolved_link_cache[link]

        app.logger.info(f"Yeni bir Vavoo linki çözülüyor: {link}")
        ...
        app.logger.info("İmza hazır, link çözme isteği gönderiliyor.")
        headers = { "user-agent": "MediaHubMX/2", "accept": "application/json", "mediahubmx-signature": signature }
        data = {"url": link, "language": "de"}
        ...
        try:
            ...
            if resolved_url:
                app.logger.info(f"Vavoo linki başarıyla çözüldü ve önbelleğe kaydedildi: {resolved_url}")
                self.resolved_link_cache[link] = resolved_url
                return resolved_url
            else:
                app.logger.warning(f"Vavoo link çözümlemesi başarılı oldu ancak yanıt bir URL içermiyordu. Yanıt: {result}")
                return None
        except Exception as e:
            app.logger.error(f"Vavoo linki çözülürken KRİTİK HATA: {e}")
            return None
            
    def resolve_vavoo_link(self, link, verbose=False):
        """Vavoo linkini önbellekten çözer veya yenisini ister (Optimize Edilmiş Versiyon)."""
        if link in self.resolved_link_cache:
        ...
        signature = self.getAuthSignature()
        if not signature:
            app.logger.error("Cannot resolve Vavoo link without a signature.")
            return None
    ...
    def resolve_vavoo_link(self, link, verbose=False):
        """Vavoo linkini önbellekten çözer veya yenisini ister."""
        if link in self.resolved_link_cache:
            app.logger.info(f"Vavoo Resolved Link CACHE HIT for: {link}")
            return self.resolved_link_cache[link]
        ...
        try:
            resp = self.session.post("https://vavoo.to/mediahubmx-resolve.json", json=data, headers=headers, timeout=45) # Timeout artırıldı
            resp.raise_for_status()
            result = resp.json()
            resolved_url = None
            if isinstance(result, list) and result and result[0].get("url"):
...
# MEVCUT @app.route('/play/...') FONKSİYONUNU KOMPLE SİLİP BU NİHAİ VERSİYONU YAPIŞTIRIN

@app.route('/play/<string:stream_id>.m3u8')
def play_vavoo_stream(stream_id):
    """
    (Nihai Stabil Versiyon)
    Vavoo linkini hızlıca çözer ve M3U8'i yeniden yazar.
    Tüm video segmentleri (.ts) 'gevent' ile optimize edilmiş proxy üzerinden akar.
    Bu yöntem hem yayınların açılmasını sağlar hem de 'ikiletme' sorununu çözer.
    """
    vavoo_url = f"https://vavoo.to/vavoo-iptv/play/{stream_id}"
    app.logger.info(f"[/play] Stabil istek başladı: {vavoo_url}")

    try:
        # 1. Vavoo linkini çöz (hızlı versiyon ile)
        resolved_url = vavoo_resolver.resolve_vavoo_link(vavoo_url, verbose=True)
        if not resolved_url:
            app.logger.error(f"[/play] Kritik Hata: Vavoo linki çözülemedi: {vavoo_url}")
            return "Vavoo linki çözülemedi", 502

        app.logger.info(f"[/play] Vavoo linki başarıyla çözüldü: {resolved_url}")

        # 2. Çözülmüş M3U8 dosyasını indir
        proxy_config = get_proxy_for_url(resolved_url)
        proxy_key = proxy_config['http'] if proxy_config else None

        m3u_response = make_persistent_request(resolved_url, timeout=20, proxy_url=proxy_key)
        m3u_response.raise_for_status()
        m3u8_content = m3u_response.text
        final_url = m3u_response.url

        # 3. M3U8 içeriğini PROXY üzerinden çalışacak şekilde yeniden yaz
        parsed_url = urlparse(final_url)
        base_url = f"{parsed_url.scheme}://{parsed_url.netloc}{parsed_url.path.rsplit('/', 1)[0]}/"
        ...
        for line in m3u8_content.splitlines():
            ...
            if line.startswith("#EXT-X-KEY"):
                # Şifreleme anahtarını proxy'le
                line = replace_key_uri(line, "")
                modified_m3u8.append(line)
            elif line and not line.startswith("#"):
                # *** ÖNEMLİ DEĞİŞİKLİK ***
                # .ts segmentlerini tekrar sunucu üzerinden proxy'liyoruz.
                # 'gevent' worker sayesinde bu işlem artık tekleme yapmayacak.
                segment_absolute_url = urljoin(base_url, line)
                line = f"/proxy/ts?url={quote(segment_absolute_url)}"
                modified_m3u8.append(line)
            else:
                modified_m3u8.append(line)

        rewritten_m3u8_content = "\n".join(modified_m3u8)
        app.logger.info(f"[/play] M3U8 başarıyla yeniden yazıldı. Segmentler proxy üzerinden akacak.")

        return Response(rewritten_m3u8_content, content_type="application/vnd.apple.mpegurl")

    except requests.exceptions.Timeout:
        app.logger.error(f"[/play] Kaynaktan zaman aşımı hatası: {vavoo_url}")
        return "Kaynak sunucudan zaman aşımı (Timeout)", 504
    except Exception as e:
        app.logger.error(f"[/play] Genel hata oluştu: {e}\n{traceback.format_exc()}")
        return f"Sunucuda beklenmedik bir hata oluştu.", 500